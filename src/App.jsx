        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {teamData.teamMembers.map((member) => (
            <button
              ...
              className="card-surface group relative flex min-h-[260px] flex-col items-center gap-4 p-8 text-center hover:border-[var(--color-accent-blue)]"
              type="button"
            >
              {member.profileImage ? (
                <img
                  ...
                  className="h-20 w-20 border-2 border-[var(--color-border)] object-cover"
                />
              ) : (
                <div className="avatar-gradient flex h-20 w-20 items-center justify-center border-2 border-[var(--color-border)] text-2xl font-semibold text-white">
                  {member.avatar}
                </div>
              )}
              <div>
...
const Dashboard = ({ theme, onToggleTheme }) => {
  ...
  return (
        ...
        <nav className="border-t border-[var(--color-border)]" aria-label="Section navigation">
          <div className="mx-auto flex max-w-[1400px] flex-wrap gap-2 px-6 py-3 text-xs font-medium text-[var(--color-text-secondary)] md:px-12">
            {sectionLinks.map((link) => (
              <a
                key={link.id}
                className="flex min-h-[44px] items-center border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 py-1 text-[var(--color-text-primary)] transition hover:border-[var(--color-accent-blue)]"
                href={`#${link.id}`}
              >
                {link.label}
              </a>
      ...
      >
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-6">
              {member.profileImage ? (
                <img
                  src={member.profileImage}
                  alt={`${member.name}, ${member.role}`}
                  className="h-[120px] w-[120px] border-2 border-[var(--color-border)] object-cover"
                />
              ) : (
                <div className="avatar-gradient flex h-[120px] w-[120px] items-center justify-center border-2 border-[var(--color-border)] text-3xl font-semibold text-white">
                  {member.avatar}
                </div>
              )}
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              {member.icp.targetRoles.map((role) => (
                <span
                  key={role}
                  className="bg-[rgba(35,55,241,0.2)] px-3 py-1 text-[13px] font-medium text-white"
                >
                  {role}
                </span>
              ))}
        ...
        <div className="mb-20 flex items-center gap-3 text-sm font-medium text-[var(--color-text-primary)]">
        ...
        <section id="starters" className={`mt-20 ${focusCard}`}>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {member.conversationStarters.map((starter) => (
              <div key={starter.id} className="card-surface p-8">
